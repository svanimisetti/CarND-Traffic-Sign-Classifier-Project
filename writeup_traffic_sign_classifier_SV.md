...
It is also interesting to note that the test set consists of images which have signficantly different aspect ratio compared to tests set (perhaps due to an angle at which the photo was captured). The accuracy of predition seemed to be good, perhpas as a result of the augmented dataset which introduced image histogram normalization and shape distortions. For example, the mandatory roundabout has an aspect ratio different from 1.0 (as was commonly observed in the images from the training set), the trained model was able to label it correctly. Another potential source of error from the custom test set was that some traffic signs were occluded (see the Priority Road sign as an example). However, the network did a good job in classifying it correctly. This is perhaps a one off case (the network got lucky?). A possible robust remedy could be that standard shape based occlusion could be introduced in the augmented databse. Overall, I think the network did a decent job.
